          ...
          <tbody>
            {
              users.map(user=>(
                <tr key={user._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <select value={user.role} onChange={(e)=>{handleRoleChange(user._id,e.target.value)}} className="p-2 border-gray-200 rounded">
                      <option value="customer">Customer</option>
                      <option value="admin">Admin</option>
                    </select>
                  </td>
                  <td className="p-4">
                    <button onClick={()=>{handleDeleteUser(user._id)}} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 cursor-pointer">Delete</button>
                  </td>
                </tr>
              ))
            }
